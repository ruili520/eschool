	...
	<div class="yo-scroll" :class="{'down':(state===0),'up':(state==1),refresh:(state===2),touch:touching}" @touchstart="touchStart($event)" @touchmove="touchMove($event)" @touchend="touchEnd($event)">
		<section class="inner" :style="{ transform: 'translate3d(0, ' + top + 'px, 0)' }">
			...
			<footer class="load-more">
				<slot name="load-more">
          <p v-if="downFlag">加载中……</p>
					<p v-else>上拉加载更多</p>
				</slot>
			</footer>
			<div class="nullData" v-show="dataList.noFlag">加载完啦！</div>
		</section>
	</div>
</template>
<script>
...
<style>
  ...
  .up-tip:before {
    background: url(../assets/img/pull.png) no-repeat center;
    background-size: 20px;
  }
  .refresh-tip:before {
    background: url(../assets/img/shua.png) no-repeat center;
